...
short_description: Resolve a Mac address to it's manufacturer
...
def run_module():
    ...
    result['manufacturer'] = getMFR(module.params['mac'])
...
""" 
    ...
    for line in mfrtext.splitlines():
        ...
        value = " ".join(re.split(r"\W",line)[1:]).strip() 
        mfrlist[key] = value
...
def getMFR(mac):
    macraw =  mac.replace(":","")
    return getMacMFRtext(macraw[0:6].upper())
